...
const FILTERS = [
  ...
  { key: "INTERESTED", label: "Intéressé",  dot: "bg-blue-500" },
  { key: "QUOTE",      label: "Devis",      dot: "bg-violet-500" },
...
export default function ProspectsTable({ rows }: { rows: ProspectRow[] }) {
  ...
  const filtered = useMemo(
    () =>
      rows.filter((r) => {
        ...
        const matchStatus = filter === "all" || normalized === filter || r.status === filter;
        const matchSearch =
          !search ||
  ...
  const countForFilter = (key: string) => {
    ...
    if (key === "INTERESTED") return rows.filter((r) => r.status === "INTERESTED" || r.status === "DEMO").length;
                ...
                >
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-1.5">
                      {PRIORITY_ICON[p.priority] && (
                        <span className="text-xs">{PRIORITY_ICON[p.priority]}</span>
                      )}
                      <div>
                        <Link
                  ...
                  <td className="px-3 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      <Link
                        href={`/espace/prospects/${p.id}`}
                        className="rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-500 px-2 py-1 text-xs font-semibold transition"
                        title="Voir détails"
                      >
                        Détails
                      </Link>
