...
// ─── E-mail : Diffusion d'opportunité aux partenaires ──────────────────────

export async function sendOpportunityBroadcastEmail(opts: {
  to: string;
  firstName: string;
  opportunityTitle: string;
  ...
  const commissionDisplay = opts.commissionType === "PERCENT"
    ? `${opts.commission}% de la valeur`
    : new Intl.NumberFormat("fr-FR", { style: "currency", currency: "XOF", maximumFractionDigits: 0 }).format(opts.commission);

  const html = layout(`
    <div style="background:linear-gradient(135deg,#0b5fff,#0040cc);border-radius:14px;padding:28px 28px 24px;margin-bottom:24px;">
      <p style="margin:0 0 4px;font-size:11px;color:rgba(255,255,255,0.75);text-transform:uppercase;letter-spacing:1px;font-weight:700;">Nouvelle opportunité IBIG PARTNERS</p>
      <h2 style="margin:0;font-size:22px;font-weight:800;color:#ffffff;line-height:1.3;">${opts.opportunityTitle}</h2>
      <p style="margin:10px 0 0;font-size:13px;color:rgba(255,255,255,0.8);">${opts.opportunityCategory}</p>
    </div>

    <p style="margin:0 0 16px;color:#5b6577;font-size:15px;line-height:1.6;">
      Bonjour <strong>${opts.firstName}</strong>, l'équipe IBIG PARTNERS partage avec vous une opportunité d'affaires
      qui pourrait correspondre à votre réseau ou à votre expertise.
    </p>
    ...
    <div style="background:#fffbeb;border-radius:10px;padding:14px 18px;margin-bottom:20px;border:1px solid #fde68a;">
      <p style="margin:0 0 4px;font-size:11px;font-weight:700;color:#92400e;text-transform:uppercase;">💬 Note IBIG</p>
      <p style="margin:0;font-size:14px;color:#92400e;">${opts.adminNote}</p>
    </div>` : ""}
    ...
    <div style="background:#f0f4ff;border-radius:12px;padding:16px 20px;margin-bottom:24px;border:1px solid #c7d2fe;">
      <p style="margin:0 0 8px;font-size:13px;font-weight:700;color:#0f1729;">Vous avez un contact qui correspond à cette opportunité ?</p>
      <p style="margin:0;font-size:13px;color:#5b6577;line-height:1.6;">
        Rendez-vous dans votre espace partenaire, onglet <strong>Opportunités</strong>, et cliquez sur
        <strong>"Je me porte volontaire"</strong>. L'équipe IBIG organisera la mise en relation.
      </p>
    </div>

    ${btn("Voir l'opportunité →", `${SITE}/espace/opportunites`)}
    <hr style="margin:28px 0;border:none;border-top:1px solid #e2e8f0;" />
    <p style="margin:0;font-size:13px;color:#94a3b8;">
      Support WhatsApp : <a href="https://wa.me/2250778882592" style="color:#0b5fff;font-weight:700;">+225 07 78 88 25 92</a>
    </p>
  ...
  return sendEmail({
    to: opts.to,
    subject: `🤝 Opportunité IBIG : ${opts.opportunityTitle}`,
    html,
  });
}
